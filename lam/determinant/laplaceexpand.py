...
def _lap_expr_0(matrix, n: int = 0, coe = 1):
    ...
    for colInd in range(startInd+1 ,matrix.shape[1]):
        if matrix[n, colInd] in (0, 1, -1):
            t = sp.MatMul(coe*matrix[n, colInd], cofactor.cofactor_submatrix(matrix, n, colInd), evaluate = True)
        else:
            t = sp.MatMul(coe*matrix[n, colInd], cofactor.cofactor_submatrix(matrix, n, colInd), evaluate = False)
...
def _lap_init_expr_0(matrix, n: int = 0, coe = 1):
    ...
    for colInd in range(matrix.shape[1]):
        if matrix[n, colInd] in (0, 1, -1): #去掉一些不必要的系数如，0，1，-1，使得输出格式化
            expr = sp.MatMul(coe*matrix[n, colInd], cofactor.cofactor_submatrix(matrix, n, colInd), evaluate = True)
        else:
            expr = sp.MatMul(coe*matrix[n, colInd], cofactor.cofactor_submatrix(matrix, n, colInd), evaluate = False)
...
def _lap_expr_1(matrix, n: int = 0, coe = 1):
    ...
    for rowInd in range(startInd+1 ,matrix.shape[0]):
        if matrix[rowInd, n] in (0, 1, -1):
            t = sp.MatMul(coe*matrix[rowInd, n], cofactor.cofactor_submatrix(matrix, rowInd, n), evaluate = True)
        else:
            t = sp.MatMul(coe*matrix[rowInd, n], cofactor.cofactor_submatrix(matrix, rowInd, n), evaluate = False)
        if not isinstance(t, sp.matrices.expressions.special.ZeroMatrix):
            expr = sp.MatAdd(expr, t, evaluate = False)
    return expr
...
def _lap_init_expr_1(matrix, n: int = 0, coe = 1):
    ...
    for rowInd in range(matrix.shape[1]):
        if matrix[rowInd, n] in (0, 1, -1): #去掉一些不必要的系数如，0，1，-1
            expr = sp.MatMul(coe*matrix[rowInd, n], cofactor.cofactor_submatrix(matrix, rowInd, n), evaluate = True)
        else:
            expr = sp.MatMul(coe*matrix[rowInd, n], cofactor.cofactor_submatrix(matrix, rowInd, n), evaluate = False)
        if not isinstance(expr, sp.matrices.expressions.special.ZeroMatrix):    #hint 0矩阵不等于0，不能用==判别
            startInd = rowInd
            break
    return (expr, startInd)
